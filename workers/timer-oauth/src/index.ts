/**
 * VPRAVA.ONLINE — Google OAuth token proxy
 * (refresh tokens in KV, access tokens to browser).
 */
...
const SESSION_COOKIE = "itt_oauth";
const SESSION_TTL_SEC = 60 * 60 * 24 * 400; // ~13 months
const COOKIE_DOMAIN = ".konashevych.com";
...
const createSessionCookie = async (
  ...
  return `${SESSION_COOKIE}=${value}; Path=/; HttpOnly; Secure; SameSite=None; Max-Age=${SESSION_TTL_SEC}; Domain=${COOKIE_DOMAIN}`;
};

const clearSessionCookie = (): string =>
  `${SESSION_COOKIE}=; Path=/; HttpOnly; Secure; SameSite=None; Max-Age=0; Domain=${COOKIE_DOMAIN}`;
